...
const translations = {
  en: {
    ...
    startFirstCall: 'Start your first call to see it here',
  ...
  ar: {
    ...
    yourCalls: 'مكالماتك',
    callsRecorded: 'مكالمات مسجلة',
    loadingCalls: 'جاري تحميل المكالمات...',
    noCalls: 'لا توجد مكالمات بعد',
    startFirstCall: 'ابدأ مكالمتك الأولى لرؤيتها هنا',
...
export default function HistoryPage() {
  ...
  const formatDate = (d: string) =>
    new Date(d).toLocaleDateString(lang === 'ar' ? 'ar-AE' : 'en-US', {
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card-active { background: var(--divider) !important; border-color: rgba(255,255,255,0.15) !important; }
        .nav-btn { transition: all 0.2s; }
        .nav-btn:hover { background: var(--divider) !important; color: rgba(255,255,255,0.7) !important; }
      ...
      <div dir={isRTL ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#000' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
          ...
          <div style={{ position: 'absolute', bottom: '-10%', right: '-10%', width: '40%', height: '40%', borderRadius: '50%', background: '#4488ff', filter: 'blur(160px)', opacity: 0.05 }} />
        </div>

        <div style={{ position: 'relative', zIndex: 10, display: 'flex', height: '100vh', padding: 16, gap: 16 }}>

          {/* Sidebar */}
          <aside style={{ width: 80, borderRadius: 32, background: 'rgba(255,255,255,0.03)', border: '1px solid var(--divider)', backdropFilter: 'blur(40px)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 0', gap: 8, flexShrink: 0 }}>
            <div onClick={() => window.location.href = '/'} style={{ width: 48, height: 48, borderRadius: 16, background: 'linear-gradient(135deg, var(--text-dim), var(--divider))', border: '1px solid rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16, cursor: 'pointer' }}>
            ...
            {[
              ...
              <div key={i} onClick={() => window.location.href = item.href} className="nav-btn" style={{ width: 48, height: 48, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: item.active ? 'rgba(255,255,255,0.1)' : 'transparent', color: item.active ? '#fff' : 'var(--text-tertiary)', border: item.active ? '1px solid rgba(255,255,255,0.15)' : '1px solid transparent' }}>
          ...
          <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
            ...
            <header style={{ height: 80, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <span style={{ fontSize: 14, color: 'var(--text-secondary)' }}>{tr.callHistory}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button onClick={() => window.location.href = '/'} style={{ height: 38, padding: '0 20px', borderRadius: 19, border: '1px solid rgba(255,255,255,0.12)', background: 'var(--divider)', color: 'rgba(255,255,255,0.8)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', backdropFilter: 'blur(20px)' }}>
                  {tr.newCall}
                </button>
              </div>
            </header>
            ...
            <div className="cs" style={{ flex: 1, overflowY: 'auto', padding: '0 8px 24px', display: 'grid', gridTemplateColumns: selected ? '1fr 400px' : '1fr', gap: 16 }}>
              ...
              <div>
                <div style={{ marginBottom: 24, padding: '0 8px' }}>
                  <div style={{ fontSize: 28, fontWeight: 600, letterSpacing: '-0.5px', color: 'var(--text-primary)', marginBottom: 4 }}>{tr.yourCalls}</div>
                  <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.35)' }}>{calls.length} {tr.callsRecorded}</div>
                </div>
                ...
                {!loading && calls.length === 0 && (
                  <div className="card" style={{ padding: '60px 40px', textAlign: 'center', cursor: 'default' }}>
                    ...
                    <div style={{ fontSize: 20, fontWeight: 600, color: 'rgba(255,255,255,0.85)', marginBottom: 8 }}>{tr.noCalls}</div>
                    <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.35)', marginBottom: 28 }}>{tr.startFirstCall}</div>
                    <button onClick={() => window.location.href = '/'} style={{ height: 44, padding: '0 28px', borderRadius: 22, border: 'none', background: 'var(--text-primary)', color: '#000', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                      {tr.startCall}
                    </button>
                  </div>
                )}
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                              <div style={{ width: 44, height: 44, borderRadius: 14, background: 'var(--divider)', border: '1px solid var(--input-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>🎙</div>
                              <div>
                                <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 3, letterSpacing: '-0.2px' }}>
                                  {call.contact_name}
                                  <span style={{ color: 'var(--text-tertiary)', fontWeight: 400 }}> · {call.company}</span>
                                </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexShrink: 0 }}>
                              {call.insights?.dealHealthScore && (
                                <div style={{ textAlign: 'center' }}>
                                  ...
                                  <div style={{ fontSize: 10, color: 'var(--text-tertiary)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{tr.health}</div>
                                </div>
                              )}
                              <div style={{ textAlign: 'center' }}>
                                <div style={{ fontSize: 18, fontWeight: 600, color: '#30d158', letterSpacing: '-0.5px' }}>{formatDuration(call.duration)}</div>
                                <div style={{ fontSize: 10, color: 'var(--text-tertiary)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{tr.duration}</div>
                              </div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                            <button
                              ...
                              style={{ height: 32, padding: '0 16px', borderRadius: 16, border: '1px solid rgba(255,255,255,0.1)', background: 'var(--divider)', color: 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}
                            >
                              {tr.viewSummary}
                            </button>
                          </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12, animation: 'fadeUp 0.25s ease' }}>
                  ...
                  <div className="card" style={{ padding: 24, position: 'relative' }}>
                    <button
                      ...
                      style={{ position: 'absolute', top: 18, [isRTL ? 'left' : 'right']: 18, width: 28, height: 28, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}
                    >✕</button>
                    <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: 10 }}>{tr.callDetails}</div>
                    <div style={{ fontSize: 20, fontWeight: 600, letterSpacing: '-0.3px', marginBottom: 4, color: 'var(--text-primary)', paddingRight: isRTL ? 0 : 36, paddingLeft: isRTL ? 36 : 0 }}>{selected.contact_name}</div>
                    <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 20 }}>{selected.company} · {formatDate(selected.created_at)}</div>
                    <div style={{ display: 'flex', gap: 10 }}>
                      {[
                        { label: tr.duration, value: formatDuration(selected.duration), color: '#30d158' },
                        { label: tr.health, value: (selected.insights?.dealHealthScore || 0) + '%', color: '#ff9f0a' },
                        { label: tr.talk, value: (selected.insights?.talkRatio || 0) + '%', color: '#0a84ff' },
                      ].map((s, i) => (
                        <div key={i} style={{ flex: 1, textAlign: 'center', padding: '12px 8px', borderRadius: 14, background: 'var(--input-bg)', border: '1px solid var(--divider)' }}>
                          <div style={{ fontSize: 20, fontWeight: 600, color: s.color, letterSpacing: '-0.5px', marginBottom: 2 }}>{s.value}</div>
                          <div style={{ fontSize: 10, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{s.label}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {selected.insights?.nextActions && (
                    <div className="card" style={{ padding: 20 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: 14 }}>{tr.nextActions}</div>
                  ...
                  {selected.insights?.objections && selected.insights.objections.length > 0 && (
                    <div className="card" style={{ padding: 20 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#ff453a', marginBottom: 14 }}>{tr.objections}</div>
                  ...
                  {selected.transcript && (
                    <div className="card" style={{ padding: 20 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: 14 }}>{tr.transcript}</div>
                  ...
                  <button
                    ...
                    style={{ width: '100%', height: 48, borderRadius: 24, border: 'none', background: 'var(--text-primary)', color: '#000', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}
