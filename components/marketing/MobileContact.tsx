    ...
    <div style={{ direction: isAr ? 'rtl' : 'ltr', fontFamily: isAr ? "'Noto Sans Arabic', sans-serif" : "'DM Sans', sans-serif" }}>
      ...
      <section style={{ padding: '40px 24px 32px', textAlign: 'center' }}>
        <h1 className="serif" style={{ fontSize: 36, fontWeight: 400, letterSpacing: '-1.5px', lineHeight: 1.1, marginBottom: 12 }}>
          {isAr ? 'دعنا ' : "Let's "}<span style={{ fontStyle: 'italic', color: '#0071e3' }}>{isAr ? 'نتحدث' : 'talk'}</span>
        </h1>
        <p style={{ fontSize: 15, color: '#6e6e73', lineHeight: 1.6 }}>
          {isAr ? 'نقوم بالرد خلال ساعتين خلال ساعات العمل.' : 'We reply within 2 hours during business hours.'}
        </p>
      </section>
      ...
      <section style={{ padding: '0 20px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <a href="mailto:hello@dealflow-ai.com" style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '20px 12px', borderRadius: 20, background: '#f5f5f7', border: '1px solid rgba(0,0,0,0.06)' }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: '#1d1d1f' }}>{isAr ? 'راسلنا' : 'Email us'}</span>
          ...
          <a href="https://wa.me/971500000000" style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '20px 12px', borderRadius: 20, background: '#f5f5f7', border: '1px solid rgba(0,0,0,0.06)' }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: '#1d1d1f' }}>{isAr ? 'واتساب' : 'WhatsApp'}</span>
            <span style={{ fontSize: 11, color: '#6e6e73', textAlign: 'center' }}>{isAr ? 'دردش معنا الآن' : 'Chat with us now'}</span>
          </a>
        </div>
      </section>
      ...
      <section style={{ padding: '0 20px 48px' }}>
        {submitted ? (
          <div style={{ textAlign: 'center', padding: '40px 20px', borderRadius: 24, background: 'rgba(52,199,89,0.06)', border: '1px solid rgba(52,199,89,0.2)' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 600, color: '#1d1d1f', marginBottom: 8 }}>{isAr ? 'تم إرسال الرسالة!' : 'Message sent!'}</div>
            <div style={{ fontSize: 14, color: '#6e6e73' }}>{isAr ? 'سنرد عليك خلال ساعتين.' : "We'll get back to you within 2 hours."}</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#1d1d1f', marginBottom: 4 }}>{isAr ? 'أرسل لنا رسالة' : 'Send us a message'}</div>
            
            <input style={inputStyle} placeholder={isAr ? "الاسم" : "Your name"} value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
            
            <input style={{...inputStyle, textAlign: 'left', direction: 'ltr'}} placeholder={isAr ? "البريد الإلكتروني" : "Email address"} type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} />
            
            <input style={inputStyle} placeholder={isAr ? "الشركة (اختياري)" : "Company (optional)"} value={form.company} onChange={e => setForm({...form, company: e.target.value})} />
            
            <div style={{ position: 'relative' }}>
              <select style={{...inputStyle, appearance: 'none', paddingInlineEnd: '32px'}} value={form.subject} onChange={e => setForm({...form, subject: e.target.value})}>
                <option value="general">{isAr ? 'استفسار عام' : 'General inquiry'}</option>
                <option value="demo">{isAr ? 'حجز عرض تجريبي' : 'Book a demo'}</option>
                <option value="enterprise">{isAr ? 'تسعير للمؤسسات' : 'Enterprise pricing'}</option>
                <option value="support">{isAr ? 'الدعم الفني' : 'Support'}</option>
              </select>
              <div style={{ position: 'absolute', [isAr ? 'left' : 'right']: '16px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', opacity: 0.5 }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M6 9l6 6 6-6"/></svg>
              </div>
            </div>

            <textarea 
              style={{...inputStyle, height: 120, padding: '12px 16px', resize: 'none'} as React.CSSProperties}
            ...
            <button 
              ...
              style={{ 
                ...
                borderRadius: 26, 
                border: 'none', 
                background: loading ? '#f5f5f7' : '#1d1d1f', 
