...
export default function MobileLanding() {
  ...
  const features = [
    { icon: '🎙', title: 'Live Transcription', desc: 'Real-time speech-to-text in English and Arabic. Every word captured instantly.', color: '#0071e3' },
    { icon: '📊', title: 'AI Deal Coach', desc: 'Real-time insights, objection handling, and deal health scores as you speak.', color: '#34c759' },
    { icon: '⚡', title: 'Buying Signals', desc: 'Never miss a signal. AI flags purchase intent in real time.', color: '#ff9f0a' },
    { icon: '💬', title: 'Smart Follow-ups', desc: 'Auto-generate personalized WhatsApp and email follow-ups instantly.', color: '#bf5af2' },
    { icon: '👥', title: 'Team Management', desc: 'Invite agents, set limits, track your whole team performance.', color: '#ff453a' },
    { icon: '📹', title: 'Google Meet & Zoom', desc: 'Works with your existing meeting tools with one click.', color: '#0071e3' },
  ]

  const plans = [
    { name: 'Solo', price: billingAnnual ? 39 : 49, color: '#0071e3', popular: false,
      features: ['Unlimited call recordings', 'Live AI coaching', 'Arabic + English', 'Smart follow-ups', 'Pre-call brief', 'Contact CRM'] },
    { name: 'Team', price: billingAnnual ? 79 : 99, color: '#bf5af2', popular: true,
      features: ['Everything in Solo', 'Up to 10 agents', 'Team dashboard', 'Per-agent controls', 'Priority support'] },
  ]

  const faqs = [
    { q: 'Does it work with Google Meet?', a: 'Yes — works with Google Meet, Zoom, and Microsoft Teams. One click to start.' },
    { q: 'How accurate is Arabic?', a: 'We use Google Cloud Speech v2 with Gulf Arabic support. Accuracy typically above 90%.' },
    { q: 'Is my data secure?', a: 'All data encrypted in transit and at rest. We never train on your calls.' },
    { q: 'Free trial details?', a: '14 days full access, no credit card required. Cancel anytime.' },
  ]
    ...
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        body { font-family: 'DM Sans', sans-serif; background: #fff; color: #1d1d1f; -webkit-font-smoothing: antialiased; }
      ...
      <section style={{ padding: '48px 24px 40px', background: '#fff', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="fade-up" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 20, background: 'rgba(0,113,227,0.08)', border: '1px solid rgba(0,113,227,0.15)', marginBottom: 24 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: '#0071e3' }}>Arabic & English supported</span>
        </div>

        <h1 className="fade-up-1 serif" style={{ fontSize: 40, fontWeight: 400, lineHeight: 1.1, letterSpacing: '-1.5px', marginBottom: 16 }}>
          Close more deals<br />with <span style={{ fontStyle: 'italic', color: '#0071e3' }}>AI that listens</span>
        </h1>

        <p className="fade-up-2" style={{ fontSize: 16, color: '#6e6e73', lineHeight: 1.6, marginBottom: 32, maxWidth: 320, margin: '0 auto 32px' }}>
          Real-time coaching, objection handling, and deal insights — in English and Arabic.
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginTop: 40, maxWidth: 340, margin: '40px auto 0' }}>
          {[['34%', 'Close rate'], ['91%', 'AR accuracy'], ['14 days', 'Free trial']].map(([val, label]) => (
      ...
      <section style={{ padding: '0 20px 40px' }}>
        <div style={{ borderRadius: 24, overflow: 'hidden', background: '#000', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ background: '#111', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginLeft: 8 }}>Live Call · Ahmed Al-Mansouri</span>
          </div>
          {/* Content */}
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 14, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginBottom: 4 }}>LIVE CALL</div>
                <div style={{ fontSize: 20, fontWeight: 600, color: '#fff', letterSpacing: '-0.5px' }}>Emirates Properties</div>
              </div>
              <div style={{ fontSize: 24, fontWeight: 300, color: '#34c759' }}>02:47</div>
            </div>
            {/* Stats row */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              {[['DEAL HEALTH', '87%', '#34c759'], ['SIGNALS', '3', '#ff9f0a'], ['SCORE', '8.4', '#0071e3']].map(([label, val, color]) => (
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 14, padding: '12px 14px' }}>
              <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', marginBottom: 6 }}>LIVE TRANSCRIPT</div>
              <div style={{ fontSize: 10, color: '#34c759', fontWeight: 700, marginBottom: 4 }}>AHMED</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', lineHeight: 1.5 }}>What's the payment plan for the 2-bedroom unit?</div>
              <div style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, background: 'rgba(48,209,88,0.1)', border: '1px solid rgba(48,209,88,0.2)' }}>
                <span style={{ fontSize: 11, color: '#34c759' }}>⚡ Buying signal detected</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="features-mobile" style={{ padding: '40px 20px', background: '#f5f5f7' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#0071e3', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10 }}>Everything you need</div>
          <h2 className="serif" style={{ fontSize: 32, fontWeight: 400, letterSpacing: '-1px', lineHeight: 1.1 }}>
            Your AI sales partner,<br /><span style={{ fontStyle: 'italic' }}>always on</span>
          </h2>
        </div>
      ...
      <section style={{ padding: '40px 20px', background: '#fff' }}>
        <div style={{ borderRadius: 24, background: 'linear-gradient(135deg, rgba(191,90,242,0.06), rgba(0,113,227,0.04))', border: '1px solid rgba(191,90,242,0.12)', padding: '28px 20px' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#bf5af2', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 12 }}>Built for the region</div>
          ...
          {['Gulf Arabic dialect support', 'Full RTL interface', 'Arabic AI coaching', 'Arabic WhatsApp follow-ups'].map((item, i) => (
      ...
      <section style={{ padding: '40px 20px', background: '#f5f5f7' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#0071e3', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10 }}>Pricing</div>
          <h2 className="serif" style={{ fontSize: 32, fontWeight: 400, letterSpacing: '-1px', marginBottom: 8 }}>
            Simple,<br /><span style={{ fontStyle: 'italic' }}>transparent pricing</span>
          </h2>
          <p style={{ fontSize: 14, color: '#6e6e73', marginBottom: 20 }}>14-day free trial. No credit card.</p>
          {/* Toggle */}
          <div style={{ display: 'inline-flex', padding: 4, borderRadius: 20, background: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            {['Monthly', 'Annual'].map((label, i) => (
              <button key={label} onClick={() => setBillingAnnual(i === 1)}
                ...
                {label}
                {i === 1 && <span style={{ fontSize: 10, fontWeight: 700, color: billingAnnual ? '#34c759' : '#34c759' }}>-20%</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {plans.map((plan, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 24, padding: '24px 20px', border: plan.popular ? `2px solid ${plan.color}` : '1px solid rgba(0,0,0,0.06)', position: 'relative' }}>
              {plan.popular && (
                <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', padding: '3px 14px', borderRadius: 12, background: plan.color, color: '#fff', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap' }}>Most Popular</div>
              )}
          ...
          <div style={{ background: '#1d1d1f', borderRadius: 24, padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#ff9f0a', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>Enterprise</div>
              <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)' }}>Custom pricing for large teams</div>
            </div>
      ...
      <section style={{ padding: '40px 20px', background: '#fff' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <h2 className="serif" style={{ fontSize: 32, fontWeight: 400, letterSpacing: '-1px', lineHeight: 1.1 }}>
            Loved by sales<br /><span style={{ fontStyle: 'italic' }}>teams in MENA</span>
          </h2>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[
            { name: 'Mohammed Al-Rashid', role: 'Senior RE Agent · Emaar', text: 'The Arabic transcription is incredibly accurate. Helped me close deals I would have lost before.', avatar: 'MA', color: '#0071e3' },
            { name: 'Sarah Thompson', role: 'Sales Director · Tech MENA', text: "Our team's close rate went up 34% in the first month. The buying signal detection is like a sixth sense.", avatar: 'ST', color: '#34c759' },
            { name: 'Khalid Al-Mansoori', role: 'Insurance Broker · Gulf', text: 'The pre-call brief alone is worth the subscription. Game changer.', avatar: 'KA', color: '#bf5af2' },
          ].map((t, i) => (
            <div key={i} style={{ background: '#f5f5f7', borderRadius: 20, padding: '20px' }}>
      ...
      <section style={{ padding: '48px 20px', background: '#1d1d1f', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
          <h2 className="serif" style={{ fontSize: 36, fontWeight: 400, letterSpacing: '-1.5px', lineHeight: 1.1, color: '#fff', marginBottom: 12 }}>
            Ready to close<br /><span style={{ fontStyle: 'italic', color: '#34c759' }}>more deals?</span>
          </h2>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', marginBottom: 28 }}>14-day free trial. No credit card.</p>
          <button className="tap-btn" onClick={() => window.location.href = '/login'}
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)' }}>No credit card · Cancel anytime</div>
        </div>
      </section>
